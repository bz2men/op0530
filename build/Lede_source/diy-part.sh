...
cat >$NETIP <<-EOF
uci set network.lan.ipaddr='10.10.10.254'                      # IPv4 地址(openwrt后台地址)
uci set network.lan.netmask='255.255.255.0'                   # IPv4 子网掩码
#uci set network.lan.gateway='10.10.10.252'                    # 旁路由设置 IPv4 网关（去掉uci前面的#生效）
#uci set network.lan.broadcast='10.10.10.255'                # 旁路由设置 IPv4 广播（去掉uci前面的#生效）
#uci set network.lan.dns='10.10.10.252'          # 旁路由设置 DNS(多个DNS要用空格分开)（去掉uci前面的#生效）
...
uci set system.@system[0].hostname='OpenWrt-5.14'              # 修改主机名称为OpenWrt-123
